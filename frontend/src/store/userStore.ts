...
interface UserStore {
  users: User[];
  addUser: (user: Omit<User, 'id' | 'pinAttempts' | 'pinLockout' | 'isRegistered'>) => void;
  removeUser: (id: string) => void;
  findUser: (firstName: string, lastName: string, company: string) => User | undefined;
          ...
          users: state.users.map(u => 
            u.id === userId 
              ? { 
                  ...
                  pinLockout: u.pinAttempts + 1 >= 3 ? Date.now() + (30 * 60 * 1000) : u.pinLockout
                } 
              : u
          )
        }));
