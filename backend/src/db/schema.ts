import { pgTable, uuid, varchar, text, timestamp, boolean, integer, decimal, jsonb, index, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const users = pgTable('users', {
  ...
  pinExpiresIdx: index('users_pin_expires_idx').on(table.pinExpiresAt),
}));
...
export const claims = pgTable('claims', {
  ...
  companyIdIdx: index('claims_company_id_idx').on(table.companyId),
  statusIdx: index('claims_status_idx').on(table.status),
  createdAtIdx: index('claims_created_at_idx').on(table.createdAt),
  claimNumberIdx: uniqueIndex('claims_claim_number_idx').on(table.claimNumber),
}));
...
export const claimNotes = pgTable('claim_notes', {
  ...
  currentIdx: index('claim_notes_current_idx').on(table.claimId, table.isCurrent),
}));
...
export const reports = pgTable('reports', {
  ...
  reportNumberIdx: uniqueIndex('reports_report_number_idx').on(table.reportNumber),
}));
